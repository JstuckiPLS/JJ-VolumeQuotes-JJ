...
/**
 * CBI processed Load report excel builder.
 * 
 * @author Alexander Nalapko
 *
 */
public class CBIProcessedLoadsExcelReportBuilder extends AbstractProcessedLoadsExcelReportBuilder {
     ...
     */
    public CBIProcessedLoadsExcelReportBuilder(ClassPathResource resource) throws IOException {
        super(resource);
    }
    ...
    private void buildHeader(ProcessedLoadsReportBO bo) {
        ...
        font.setBold(true);
        fillCellValue(mainSheet.getRow(0), 1, bo.getFailed(), "{failed}", null, font);
        fillCellValue(mainSheet.getRow(0), 0, bo.getSuccessful(), "{successful}", null, font);
        fillCellValue(mainSheet.getRow(0), 2, bo.getInvoice(), "{invoice}", null, font);
        fillCellValue(mainSheet.getRow(0), 3, bo.getRevenue(), "{revenue}", null, font);
        fillCellValue(mainSheet.getRow(0), 4, bo.getCost(), "{cost}", null, font);
