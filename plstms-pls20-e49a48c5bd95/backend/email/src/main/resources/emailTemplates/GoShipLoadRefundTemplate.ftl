...
<div>
    This shipment was paid for by GoShip user <b><#if user??>${user.firstName!" "} ${user.lastName!" "}</#if></b>, company <b><#if organization??>${organization.name!" "}, ${organization.contactEmail!""}</#if></b>.
    <br>
    But due to a failure wasn't automatically created in GoShip.  
    <br>
    <#if refund??> A refund was issued to your credit card for the Shipment.</#if>
    <#if voided??> The charge to your credit card for the Shipment was voided.</#if>
    <br><br>
    1.<b>Products</b>
    <br>
    <p>
