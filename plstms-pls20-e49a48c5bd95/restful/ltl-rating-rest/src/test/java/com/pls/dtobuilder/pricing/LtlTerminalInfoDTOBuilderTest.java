...
/**
 * Tests for {@link LtlTerminalInfoDTOBuilder}.
 * 
 * @author Artem Arapov
 * 
 */
public class LtlTerminalInfoDTOBuilderTest {

    private final LtlTerminalInfoDTOBuilder builder = new LtlTerminalInfoDTOBuilder();

    @Test
    public void testBuildDTO() {
        LtlPricingTerminalInfoEntity entity = createEntity();
        LtlTerminalInfoDTO dto = builder.buildDTO(entity);

        assertEquals(entity.getId(), dto.getId());
        assertEquals(entity.getAccountNum(), dto.getAccountNum());
        assertEquals(entity.getContactName(), dto.getContactName());
        assertEquals(entity.getEmail(), dto.getEmail());
        assertEquals(entity.getPriceProfileId(), dto.getProfileId());
        assertEquals(entity.getTerminal(), dto.getTerminal());
        assertEquals(entity.getTransiteTime(), dto.getTransiteTime());
    ...
    public void testBuildEntity() {
        LtlTerminalInfoDTO dto = createDTO();
        LtlPricingTerminalInfoEntity entity = builder.buildEntity(dto);

        assertEquals(dto.getId(), entity.getId());
        assertEquals(dto.getAccountNum(), entity.getAccountNum());
        assertEquals(dto.getContactName(), entity.getContactName());
        assertEquals(dto.getEmail(), entity.getEmail());
        assertEquals(dto.getProfileId(), entity.getPriceProfileId());
        assertEquals(dto.getTerminal(), entity.getTerminal());
        assertEquals(dto.getTransiteTime(), entity.getTransiteTime());
    ...
    private LtlPricingTerminalInfoEntity createEntity() {
        ...
        entity.setFax(fax);
        entity.setContactName("Contact" + Math.random());
        entity.setEmail("Email" + Math.random());
        entity.setPriceProfileId(1L);
        entity.setStatus(Status.ACTIVE);
        entity.setTerminal("terminal" + Math.random());
        entity.setTransiteTime(123L);
